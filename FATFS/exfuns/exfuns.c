...
extern u8 rec_sai_fifo_read(u8 **buf);  //ÕâÀïµÄbufÒª×¢ÒâßÏ£¬ËüÊÇÊ²Ã´£¿¶şÎ¬Ö¸Õë
...
void sd_creatfile(void)
{
	...
	char *sDatas ="-------------------system reset-----------------\r\n";
		//initÖĞ½øĞĞFIFOÄÚ´æÉêÇë
	for(i=0;i<RB_FIFO_SIZE;i++)
	{
		sairecfifobuf[i]=mymalloc(SRAMIN,RB_FIFO_FRAME);//SAIÂ¼ÒôFIFOÄÚ´æÉêÇë
		if(sairecfifobuf[i]==NULL)break;			//ÉêÇëÊ§°Ü
	}
 	...
 	{	
		...
		f_open(&fileobj, "0:SaveData/GpsImuData.txt", FA_CREATE_NEW);			//´´½¨¸ÃÄ¿Â¼ 			
	}
	...
	f_close(&fileobj);
  //	for(i=0;i<RB_FIFO_SIZE;i++)myfree(SRAMIN,sairecfifobuf[i]);//SAIÂ¼ÒôFIFOÄÚ´æÊÍ·Å
	
}


//void sd_savegps(u8 * filepath,u8 * datas,u32 length)
void sd_savefifo(u8 * filepath,u8 frame)   //frame Ö¡
{
	FIL fileobj;   //ÎÄ¼ş¶ÔÏó£¬±£´æµ±Ç°²Ù×÷µÄÎÄ¼ş×´¿ö
	FRESULT fr;
	UINT brs;
...
///	
	...
	if(fr==FR_OK)
	{
		...
		for(i = 0;i<frame;i++)  //¶ÁÈ¡Êı¾İÖ¡Êı
		{
			if(rec_sai_fifo_read(&pdatabuf))//¶ÁÈ¡Ò»´ÎÊı¾İ,¶Áµ½Êı¾İÁË,Ğ´ÈëÎÄ¼ş
				{
					//res=f_write(&fileobj,pdatabuf,52,(UINT*)&brs);//Ğ´ÈëÎÄ¼ş
					f_printf(&fileobj, "%s\n", pdatabuf); 
					
					memset(pdatabuf,'\0',RB_FIFO_FRAME);  //½«¶ÁÍêµÄÊı×éÇå¿Õ£¬·ÀÖ¹ºóĞøÊ¹ÓÃÊ±£¬ĞÂµÄ½Ï¶ÌµÄ×Ö·û´®ÎŞ·¨½«ºó±ßµÄ¾ÉÖµ¸²¸Çµô
...
void sd_savesl(u8 * filepath,u8 * datas,u32 length)   //±£´æÉè¶¨³¤¶ÈÊı¾İµ½SD¿¨  set length
{
		FIL fileobj;   //ÎÄ¼ş¶ÔÏó£¬±£´æµ±Ç°²Ù×÷µÄÎÄ¼ş×´¿ö
		FRESULT fr;
		UINT brs;
